...
public class FrameOfReferenceTableParser implements TableParser<List<FrameOfReference>, FrameOfReferenceTableRowParser> {
    @Override
    public List<FrameOfReference> parse(@NotNull Element element, @NotNull FrameOfReferenceTableRowParser rowParser) throws ParserException, XPathExpressionException, IllegalArgumentException, NullPointerException {
        var registry = new ArrayList<FrameOfReference>();
        build(element, registry, rowParser, "table_A-2");
        return registry;
    }

    private void build(@NotNull Element element, @NotNull List<FrameOfReference> registry, @NotNull FrameOfReferenceTableRowParser rowParser, String tableId) throws XPathExpressionException, ParserException {
        // Find table by id.
